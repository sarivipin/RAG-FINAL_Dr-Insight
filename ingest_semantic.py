"""
Ingestion using Semantic Chunking.
Uses the embedding model to detect meaning shifts in text and splits
at semantic boundaries. Groups semantically similar sentences together.
"""
import os
import re
import shutil
...
# =========================================================
DOCS_PATH = "docs"
PERSIST_DIRECTORY = "db/chroma_db_semantic"
COLLECTION_NAME = "medical_semantic_chunks"

EMBEDDING_MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"

# Breakpoint threshold: percentile means split when similarity drops
# below the Xth percentile of all similarities
...
def chunk_documents(documents: List[Document]) -> List[Document]:
    print(f"Splitting with SemanticChunker (breakpoint={BREAKPOINT_TYPE}, threshold={BREAKPOINT_THRESHOLD})...")
    ...
    for doc_idx, doc in enumerate(documents):
        ...
        # SemanticChunker needs enough text to work with
        if len(content) < 100:
            chunks.append(Document(
                page_content=content,
                metadata={
                    "chunk_id": hashlib.md5(f"{source_path}_0".encode()).hexdigest()[:16] + "_s0",
                    "source": source_path,
                    "disease": disease,
                    "section": "general",
                    "source_url": "",
        ...
        try:
            semantic_docs = chunker.create_documents([content])
        except Exception:
            # Fallback: treat entire doc as one chunk
            semantic_docs = [Document(page_content=content)]

        for i, sdoc in enumerate(semantic_docs):
            chunk_id = hashlib.md5(f"{source_path}_{i}".encode()).hexdigest()[:16]
            section = extract_section(sdoc.page_content)
            source_url = extract_source_url(sdoc.page_content)

            chunks.append(Document(
                page_content=sdoc.page_content,
                metadata={
        ...
        if (doc_idx + 1) % 50 == 0:
            print(f"  Processed {doc_idx + 1}/{len(documents)} files...")
...
# =========================================================
def main():
    print("=== Ingestion: Semantic Chunking ===\n")
