...
def _make_groq_llm(temperature: float = 0.2) -> ChatGroq:
    return ChatGroq(
        model=GROQ_MODEL,
...
# Patch retrieval so the RAG pipeline also uses Groq with retry
class _RetryLLMWrapper:
    """Wraps a ChatGroq so .invoke() retries on rate limits."""
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if "429" in str(e) or "rate" in str(e).lower():
                    ...
                    raise
        raise RuntimeError(f"RAG LLM failed after {MAX_RETRIES} retries.")

    def __getattr__(self, name):
        return getattr(self._llm, name)


retrieval.load_llm = lambda: _RetryLLMWrapper(_make_groq_llm(temperature=0.2))
...
# =========================================================
def evaluate_faithfulness(llm: ChatGroq, response: str, contexts: List[str]) -> float:
...
def evaluate_context_recall(llm: ChatGroq, response: str, contexts: List[str], reference: str) -> float:
...
def run_evaluation(rows: List[Dict]) -> Dict:
    llm = _make_groq_llm(temperature=0.0)
...
def main() -> None:
    ...
    print("Running RAG pipeline (Groq)...")
    rows = run_rag_on_questions(questions, reference_map)

    print("\nRunning evaluation (Groq + BLEU)...")
